...
- 修复节点组转发实际有流量但前端显示 `0B`：节点端已持续上报 `1_1_0_tcp` 流量，后端因老库 `forward` 表缺少 `daily_in_flow/daily_out_flow`，导致 `FlowController.updateForwardFlow()` 的整条累加 SQL 失败，`forward/user` 总流量也没有入库。`gost.sql` 和 `panel_install.sh` 补齐 `forward.daily_in_flow/daily_out_flow` 幂等迁移，`tests/daily_flow_limit_check.mjs` 覆盖 forward/user/user_tunnel 三处日计数字段。生产库已补列，后续上报已入库：`forward id=1` 与 `user id=1` 当前 `in_flow=15705828`、`out_flow=81439028`。自检：`node tests/daily_flow_limit_check.mjs`、`node tests/aggregate_forward_ui_check.mjs`、`bash tests/panel_install_fix_check.sh`、`bash -n panel_install.sh`、前端 `npm run build` 通过。
