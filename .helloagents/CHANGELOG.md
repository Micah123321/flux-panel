...
- 修正聚合转发业务模型：`/aggregate-forward` 页面收敛为节点组管理，移除独立“新增聚合转发/转发规则”入口；后端 `AggregateForwardServiceImpl` 下线 create/update/resume，仅保留 legacy delete 清理历史服务。隧道管理新增节点组选择，`Tunnel`/`TunnelDto`/`TunnelListDto` 增加 `inGroupId/outGroupId` 与组名返回；普通转发按隧道节点组展开 GOST 服务，端口分配改为组内共同空闲端口，chain 支持出口组多节点 selector。安装脚本和 `gost.sql` 增加 `tunnel.in_group_id/out_group_id` 幂等迁移。自检：`node tests/aggregate_forward_ui_check.mjs`、前端 `npm run build` 通过；本机无 `mvn`/Maven Wrapper，未执行本地后端编译。
