...
- 节点组列表返回成员节点的 `portSta`/`portEnd`，前端可展示节点组公共端口范围。
- 新增聚合转发弹窗默认选中前两个节点组（只有一个节点组时入口/出口复用同组），名称、入口地址、入口端口与出口端口会基于入口组自动填充；入口端口取入口组成员端口范围交集的起始端口。
- 入口 IP/域名由用户手动填写或从入口组节点地址候选中点选，支持逗号、空格、换行分隔；系统不托管 DNS、TLS 证书或反代。
