...
- 节点组成员允许在被隧道引用时动态修改：保存成员后刷新引用隧道的首节点兼容字段，并调用普通转发与历史聚合转发同步；新增成员会 update/add 对应服务，移除成员会删除其节点上的服务。
- 节点组离线成员会自动剔除：节点 WebSocket 断开时会从所有节点组移除该节点；节点组列表、隧道创建和转发创建/更新也会先清理历史离线成员。离线节点被移除时不再向其发送删除命令，剩余在线成员会继续同步服务。
- 转发列表会展开显示入口组全部在线入口地址；转发/隧道诊断都会覆盖组内入口节点，避免看起来只有首个节点生效。
- 端口分配按组内在线成员的公共端口范围找共同空闲端口，避免只检查首个节点导致组内冲突。
## 历史数据清理
