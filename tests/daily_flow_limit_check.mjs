...
// 1. 数据库层：package_plan / user / user_tunnel 三表均有日限与日计数字段；forward 有日计数字段供上报 SQL 累加
...
for (const [table, column] of [['forward', 'daily_in_flow'], ['forward', 'daily_out_flow'], ['user', 'daily_flow'], ['user', 'daily_in_flow'], ['user', 'daily_out_flow'], ['user_tunnel', 'daily_flow'], ['user_tunnel', 'daily_in_flow'], ['user_tunnel', 'daily_out_flow']]) {
...
assert.equal(schema.match(/`daily_in_flow`/g)?.length, 3, 'daily_in_flow 应出现在 forward/user/user_tunnel 三处');
assert.equal(schema.match(/`daily_out_flow`/g)?.length, 3, 'daily_out_flow 应出现在 forward/user/user_tunnel 三处');
